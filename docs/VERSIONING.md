...
|---|---|---|
| **Plan** | reads the version, looks for its tag | the tag exists, so nothing to release, and the run says so |
...
| | `gh release create --target <commit>` | nothing; this is the last step |
...
To release by hand, including doing the bump commit for you, use **Actions > release > Run
workflow**, which takes a `bump` of `patch`/`minor`/`major`, an
